...
class OutlinePass extends Pass {
	...
	constructor(resolution: any, scene: any, camera: any, selectedObjects: any) {
		...
		this.renderTargetMaskBuffer = new WebGLRenderTarget(this.resolution.x, this.resolution.y, {
			type: UnsignedByteType, // or UnsignedByteType depending on renderer
			format: RGBAFormat,
		...
		this.materialCopy = new ShaderMaterial({
			uniforms: this.copyUniforms as any,
		...
		function replaceDepthToViewZ(string: any, camera: any) {
	...
	render(renderer: any, _writeBuffer: any, readBuffer: any, _deltaTime: any, maskActive: any) {
		if (this.selectedObjects.length > 0) {
			...
			;(this.copyUniforms as any)['tDiffuse'].value = this.renderTargetMaskBuffer.texture
		...
		if (this.renderToScreen) {
			this.fsQuad.material = this.materialCopy
			;(this.copyUniforms as any)['tDiffuse'].value = readBuffer.texture
	...
	getSeparableBlurMaterial(maxRadius: number) {
		return new ShaderMaterial({
			defines: {
